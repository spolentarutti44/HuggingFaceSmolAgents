...
def search_item_ctrl_f(text: str, nth_result:int = 1) -> str:
   """
    ...
    """
   elements = driver.find_by_element(By.XPATH, f"//*[contains(text(), '{text}')]")
   if nth_result > len(elements):
        raise Exception(f"Match n°{nth_result} not found (only {len(elements)} matches found)")
   result = f"Found {len(elements)} matches for '{text}'."
   elemn = elements[nth_result - 1]
...
def save_screenshot(step_log:ActionStep, agent:CodeAgent) -> None:
    ...
    Args:
        step_log: The current action step object to attach the screenshot to.
        agent: The agent instance whose logs are being processed.
    """
    sleep(1.0)
    ...
    if driver is not None:
        for step_logs in agent.logs:
            if isinstance(step_logs, ActionStep) and steplog.step_number <= current_step -2:
